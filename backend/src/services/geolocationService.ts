...
export class GeolocationService {
  ...
  private static facilityLocations: Record<string, FacilityLocation> = {
  ...
  static calculateTripCost(distance: number, transportLevel: string): number {
    ...
    const rate = baseRate[transportLevel as keyof typeof baseRate] || baseRate['BLS'];
   ...
   */
  static getTripLocationData(
    fromLocation: string, 
    toLocation: string, 
    transportLevel: string
  ): TripLocationData | null {
    ...
    // Calculate distance
    const distance = DistanceService.calculateDistance(originLocation, destinationLocation);
