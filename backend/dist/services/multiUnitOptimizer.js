...
class MultiUnitOptimizer {
    constructor(revenueOptimizer, backhaulDetector, constraints) {
     ...
     */
    async optimizeMultiUnit(units, requests, currentTime = new Date()) {
        ...
        const optimizationMatrix = this.createOptimizationMatrix(availableUnits, pendingRequests, currentTime);
     ...
     */
    createOptimizationMatrix(units, requests, currentTime) {
        const matrix = [];
     ...
     */
    optimizeWithBackhaul(matrix, backhaulPairs, units, requests) {
        const assignments = [];
        ...
        for (const [unitId, assignedRequestsList] of unitAssignments) {
            if (assignedRequestsList.length > 0) {
                const revenue = assignedRequestsList.reduce((sum, req) => sum + this.revenueOptimizer.calculateRevenue(req), 0);
     ...
     */
    optimizeWithoutBackhaul(matrix, units, requests) {
        const assignments = [];
        const assignedRequests = new Set();
        const unitAssignments = new Map();
        // Initialize unit assignments
     ...
     */
    calculateAssignmentEfficiency(requests, deadheadMiles, waitTime) {
        if (requests.length === 0)
            return 0;
        const totalRevenue = requests.reduce((sum, req) => sum + this.revenueOptimizer.calculateRevenue(req), 0);
     ...
     */
    calculateLoadedMilesForRequests(requests) {
        return requests.reduce((sum, req) => {
            return sum + this.revenueOptimizer.calculateDistance(req.originLocation, req.destinationLocation);
        }, 0);
    }
    /**
     * Calculate loaded miles for assignments
     */
    calculateLoadedMiles(assignments) {
     ...
     */
    calculatePairedTripsPercentage(assignments, backhaulPairs) {
        const totalRequests = assignments.reduce((sum, assignment) => sum + assignment.assignedRequests.length, 0);
        if (totalRequests === 0)
            return 0;
     ...
     */
    calculateAverageResponseTime(assignments, currentTime) {
...
//# sourceMappingURL=multiUnitOptimizer.js.map