...
class BackhaulDetector {
    constructor(maxTimeWindow = 90, // 90 minutes as per reference documents
    maxDistance = 15, // 15 miles as per reference documents
    revenueBonus = 25.0 // $25 bonus for backhaul pairs
    ) {
     ...
     */
    hasCompatibleTransportLevels(request1, request2) {
     ...
     */
    calculateTimeWindow(request1, request2) {
     ...
     */
    isReturnTripScenario(request1, request2) {
     ...
     */
    calculateEfficiency(request1, request2, distance, timeWindow, isReturnTrip = false) {
     ...
     */
    calculatePriorityScore(request1, request2) {
        ...
        const priority1 = priorityValues[request1.priority] || 1;
        const priority2 = priorityValues[request2.priority] || 1;
     ...
     */
    calculateRevenueScore(request1, request2) {
        const baseRates = {
            'BLS': 150.0,
            'ALS': 250.0,
            'CCT': 400.0
        };
        const revenue1 = baseRates[request1.transportLevel] || 150.0;
        const revenue2 = baseRates[request2.transportLevel] || 150.0;
     ...
     */
    findBestPairsForRequest(request, allRequests) {
        const otherRequests = allRequests.filter(req => req.id !== request.id && req.status === 'PENDING');
        const pairs = [];
     ...
     */
    findReturnTripOpportunities(request, allRequests) {
        const otherRequests = allRequests.filter(req => req.id !== request.id && req.status === 'PENDING');
        const returnTrips = [];
        for (const otherRequest of otherRequests) {
     ...
     */
    findAllReturnTripOpportunities(allRequests) {
     ...
     */
    getBackhaulStatistics(requests) {
        const pendingRequests = requests.filter(req => req.status === 'PENDING');
        ...
        const averageEfficiency = pairs.length > 0
            ...
            : 0;
        const potentialRevenueIncrease = pairs.reduce((sum, pair) => sum + this.calculatePairingRevenue(pair), 0);
        return {
            totalRequests,
            possiblePairs,
            validPairs,
     ...
     */
    updateParameters(maxTimeWindow, maxDistance, revenueBonus) {
...
//# sourceMappingURL=backhaulDetector.js.map